import { A as onMounted, G as effectScope, J as isRef, Q as ref, V as watch, _ as defineComponent, a as Fragment, c as Text, et as shallowRef, g as createVNode, j as onUnmounted, l as computed, lt as process$1, v as getCurrentInstance, x as inject, y as h } from "./vue.esm-bundler-DdwSdRj_.js";
...
function createCompileError(code, loc, options = {}) {
	...
	const msg = process$1.env.NODE_ENV !== "production" ? format$1((messages || errorMessages$2)[code] || "", ...args || []) : code;
...
function minify(node) {
	...
	switch (node.type) {
		...
		default: if (process$1.env.NODE_ENV !== "production") throw createCompileError(CompileErrorCodes.UNHANDLED_MINIFIER_NODE_TYPE, null, {
...
function generateNode(generator, node) {
	...
	switch (node.type) {
		...
		case 3:
			generator.push(JSON.stringify(node.value), node);
			break;
		default: if (process$1.env.NODE_ENV !== "production") throw createCompileError(CompileErrorCodes.UNHANDLED_CODEGEN_NODE_TYPE, null, {
			domain: ERROR_DOMAIN,
			args: [node.type]
		});
	}
...
function compile(message, context) {
	if (!__INTLIFY_DROP_MESSAGE_COMPILER__ && isString(message)) {
		...
		process$1.env.NODE_ENV !== "production" && checkHtmlMessage(message, warnHtmlMessage);
		const cacheKey = (context.onCacheKey || defaultOnCacheKey)(message);
		const cached = compileCache[cacheKey];
		if (cached) return cached;
		const { ast, detectError } = baseCompile(message, {
			...context,
			location: process$1.env.NODE_ENV !== "production",
			jit: true
		});
		const msg = format(ast);
		return !detectError ? compileCache[cacheKey] = msg : msg;
	} else {
		if (process$1.env.NODE_ENV !== "production" && !isMessageAST(message)) {
...
function createCoreError(code) {
	return createCompileError(code, null, process$1.env.NODE_ENV !== "production" ? { messages: errorMessages$1 } : void 0);
}
/** @internal */
var errorMessages$1 = {
...
function createCoreContext(options = {}) {
	...
	if (process$1.env.NODE_ENV !== "production" && isFunction(options.messageCompiler)) warnOnce(getWarnMessage$1(CoreWarnCodes.EXPERIMENTAL_CUSTOM_MESSAGE_COMPILER));
	...
	if (process$1.env.NODE_ENV !== "production") context.__v_emitter = internalOptions.__v_emitter != null ? internalOptions.__v_emitter : void 0;
	if (process$1.env.NODE_ENV !== "production" || false) initI18nDevTools(context, version, __meta);
	return context;
}
...
function handleMissing(context, key, locale, missingWarn, type) {
	...
	if (missing !== null) {
		...
		if (process$1.env.NODE_ENV !== "production" && isTranslateMissingWarn(missingWarn, key)) onWarn(getWarnMessage$1(CoreWarnCodes.NOT_FOUND_KEY, {
			key,
			locale
		}));
		return key;
...
function datetime(context, ...args) {
	...
	if (process$1.env.NODE_ENV !== "production" && !Availabilities.dateTimeFormat) {
	...
	for (let i = 0; i < locales.length; i++) {
		...
		if (process$1.env.NODE_ENV !== "production" && locale !== targetLocale && isTranslateFallbackWarn(fallbackWarn, key)) onWarn(getWarnMessage$1(CoreWarnCodes.FALLBACK_TO_DATE_FORMAT, {
			key,
			target: targetLocale
		}));
		if (process$1.env.NODE_ENV !== "production" && locale !== targetLocale) {
			const emitter = context.__v_emitter;
...
function number(context, ...args) {
	...
	if (process$1.env.NODE_ENV !== "production" && !Availabilities.numberFormat) {
	...
	for (let i = 0; i < locales.length; i++) {
		targetLocale = to = locales[i];
		if (process$1.env.NODE_ENV !== "production" && locale !== targetLocale && isTranslateFallbackWarn(fallbackWarn, key)) onWarn(getWarnMessage$1(CoreWarnCodes.FALLBACK_TO_NUMBER_FORMAT, {
			key,
			target: targetLocale
		}));
		if (process$1.env.NODE_ENV !== "production" && locale !== targetLocale) {
...
function translate(context, ...args) {
	...
	if (process$1.env.NODE_ENV !== "production" && isString(format) && context.messageCompiler == null) {
...
function resolveMessageFormat(context, key, locale, fallbackLocale, fallbackWarn, missingWarn) {
	...
	for (let i = 0; i < locales.length; i++) {
		targetLocale = to = locales[i];
		if (process$1.env.NODE_ENV !== "production" && locale !== targetLocale && !isAlmostSameLocale(locale, targetLocale) && isTranslateFallbackWarn(fallbackWarn, key)) onWarn(getWarnMessage$1(CoreWarnCodes.FALLBACK_TO_TRANSLATE, {
			key,
			target: targetLocale
		}));
		if (process$1.env.NODE_ENV !== "production" && locale !== targetLocale) {
		...
		let startTag;
		let endTag;
		if (process$1.env.NODE_ENV !== "production" && inBrowser) {
...
function createI18nError(code, ...args) {
	return createCompileError(code, null, process$1.env.NODE_ENV !== "production" ? {
		...
		if (!key.includes(".")) {
			...
			for (let i = 0; i < lastIndex; i++) {
				...
				if (!isObject(currentObj[subKeys[i]])) {
					process$1.env.NODE_ENV !== "production" && warn(getWarnMessage(I18nWarnCodes.IGNORE_OBJ_FLATTEN, { key: subKeys[i] }));
					hasStringValue = true;
					break;
				}
...
function createComposer(options = {}) {
	...
	const getCoreContext = () => {
		...
		if (process$1.env.NODE_ENV !== "production") ctxOptions.__v_emitter = isPlainObject(_context) ? _context.__v_emitter : void 0;
	...
	const wrapWithDeps = (fn, argumentParser, warnType, fallbackSuccess, fallbackFail, successCondition) => {
		...
		if (warnType !== "translate exists" && isNumber(ret) && ret === -1 || warnType === "translate exists" && !ret) {
			...
			if (process$1.env.NODE_ENV !== "production" && __root && isString(key) && isResolvedTranslateMessage(warnType, arg2)) {
...
function createVueI18n(options = {}) {
	...
	const vueI18n = {
		...
		tc(...args) {
			...
			let list = null;
			let named = null;
			if (process$1.env.NODE_ENV !== "production") warnOnce(getWarnMessage(I18nWarnCodes.DEPRECATE_TC));
...
function defineMixin(vuei18n, composer, i18n) {
	return {
		...
		mounted() {
			/* istanbul ignore if */
			if ((process$1.env.NODE_ENV !== "production" || false) && this.$el && this.$i18n) {
				const _vueI18n = this.$i18n;
		...
		unmounted() {
			...
			const _vueI18n = this.$i18n;
			/* istanbul ignore if */
			if ((process$1.env.NODE_ENV !== "production" || false) && this.$el && this.$el.__VUE_I18N__) {
				if (this.__v_emitter) {
...
function createI18n(options = {}, VueI18nLegacy) {
	...
	const symbol = /* @__PURE__ */ makeSymbol(process$1.env.NODE_ENV !== "production" ? "vue-i18n" : "");
	...
	const i18n = {
		...
		async install(app, ...options) {
			if ((process$1.env.NODE_ENV !== "production" || false) && true) app.__VUE_I18N__ = i18n;
			...
			if ((process$1.env.NODE_ENV !== "production" || false) && true) {
...
function useI18n(options = {}) {
	...
	if (scope === "parent") {
		...
		if (composer == null) {
			if (process$1.env.NODE_ENV !== "production") warn(getWarnMessage(I18nWarnCodes.NOT_FOUND_PARENT_SCOPE));
			composer = gl;
		}
		return composer;
	}
...
function setupLifeCycle(i18n, target, composer) {
	let emitter = null;
	onMounted(() => {
		if ((process$1.env.NODE_ENV !== "production" || false) && target.vnode.el) {
	...
	onUnmounted(() => {
		const _composer = composer;
		if ((process$1.env.NODE_ENV !== "production" || false) && target.vnode.el && target.vnode.el.__VUE_I18N__) {
...
var i18n_default$1 = { "en-US": {
	failed: "Action failed",
	success: "Action was successful"
} };
//#endregion
//#region src/boot/i18n.js
var i18n_default = ({ app }) => {
	const i18n = createI18n({
		locale: "en-US",
		legacy: false,
		messages: i18n_default$1
	});
	app.use(i18n);
};
//#endregion
