...
void sendeWertAnEmpfaenger(auto inputValue) {
    Serial.println("sende Wert '" + String(inputValue) + "' an Empfänger.");
    Udp.beginPacket(WiFi.gatewayIP(), udpPort);
    // wir senden den inputValue als Text.
    Udp.write(String(inputValue).c_str());
    Udp.endPacket();
}
...
void loop() {
    int inputNow = map(analogRead(inputPin), 0, 1023, 0, 99);
